...
class Krb5PrincipalsRestHandler(DelegatingRestHandler):
  ...
  def handleList(self, confInfo):
      ...
      for name, obj in confInfo.items():
          ...
          obj['valid'] = '1' if krb5p.isValid() else '0'
          obj['principal_dir']  = p
  ...
  def handleCustom(self, confInfo):
      ...
      if self.customAction == "renew":
           ...
           if not krb5p.isValid():
              raise admin.BadActionException("Cannot execute renew without a keytab file. Copy one in: %s" % (krb5p.getKeytabFile()))
  ...
  def handleCreateOrEdit(self, confInfo):
      ...
      if not os.path.exists(keytab_path):
          raise admin.BadActionException("keytab_path does not exist: "+keytab_path)
