...
class Krb5Principal:
      ...
      def isValid(self):
          return os.path.exists(self.getKeytabFile())
      ...
      def setupEnv(self, env=None, tgtRenewTtl=None):
          ...
          raise Exception('Error while executing command="%s", error="%s"' % (' '.join(kinit_args), output[1]))
      ...
      def createProcess(self, kinit_args, env):
          ...
          try:
              ...
              raise Exception("Failed to setup kerberos environment, could not find required command: kinit")
