...
class StatusUpdateHandler(object):
    ...
    def updateDoneStatus(self, et, lt):
        ...
        try:
            ...
            logger.exception('Failed to read export job status for export_name="%s" : %s ' % (self.name, str(e)) )    
    ...
    def updateStatus(self):
        if self.name!=None and self.namespace!=None and self.owner!=None:            
            try:
                ...
                logger.exception('Failed to update export status for export_name="%s": %s ' % (self.name, str(e)))    
...
def cleanSearchString(search):
    ...
    if s.startswith('|'):
       ...
       if not s.startswith('search '):
          raise Exception('Invalid search string. Only searches whose first command is "search" can be exported')
...
def buildSearchString(splunk_servers, index_times, search, evalSearch, sid, basefilename, name, roll_size, dst, principal, format, export_fields, required_fields):
    return 'search ' + splunk_servers + ' ' + index_times + ' ' + cleanSearchString(search) + evalSearch + \
           (' | fields _dstpath %s | exporthdfs parentsid=%s basefilename=%s exportname="%s" rollsize=%d dst=%s %s format=%s fields="%s"' 
            % (required_fields, sid, basefilename, name, roll_size, dst, principal, format, export_fields))
...
class RunExport(object):        
    ...
    def createCursor(self, name, lt, dst, krb5_principal=None):
        ...
        hj.touchz(path)
        if hj.wait() != 0 or not hj.exists(path):
           raise Exception("Failed to create cursor file: %s. %s" % (path, hj.getStderr()))
        return path

    def incrCursorState(self, path, krb5_principal=None):
        m = cursor_re.search(path) 
        if m == None or m.group(3) == None:
           raise Exception("Invalid cursor path: " + path)
        ...
        hj.mv(path, new_path)
        if hj.wait() != 0:
           raise Exception("Failed to increment cursor state: %s. %s" % (path, hj.getStderr()))
        return new_path
    ...
    def finalizeCursor(self, path, krb5_principal=None):
        ...
        hj.mv(path, new_path)
        if hj.wait() != 0:
           raise Exception("Failed to finalize cursor state: %s. %s" % (path, hj.getStderr()))
        return new_path
    ...
    def removeFile(self, path, krb5_principal=None):
        ...
        hj.rm(path)
        if hj.wait() != 0:
           raise Exception("Failed to remove file %s. %s" % (path, hj.getStderr()))
    ...
    def parseCursorSearchOutput(self, abspath, krb5_principal=None):
        ...
        if hj.wait() != 0 and hj.getStderr().find('No such file or directory') < 0:
           raise Exception("Failed to list all cursors in %s: %s" % (abspath,  hj.getStderr()))
    ...
    def makeHdfsDir(self, dst, krb5_principal=None):
        hj = self.createHadoopCliJob(dst, krb5_principal)
        hj.mkdir(dst)
        hj.wait() # assume success (it either exists, we created it or we cant create it)  
    ...
    def updateReplication(self, name, replication, base, dst, krb5_principal, cursor_replication=3):
        ...
        if replication != 0:
           ...
           if hj.wait() != 0:
              raise Exception("Error while waiting for update replication job to finish. %s" % hj.getStderr())

        # update cursor replication if different than data 
        if cursor_replication != replication:
           for c in self.getCursors(name, dst):
               hj.setrep(hdfsPathJoin(base, c['path']), cursor_replication, False, False)
               if hj.wait() != 0:
                  raise Exception("Error while waiting for cursor file replication job to complete. %s" % hj.getStderr())
    ...
    def renameTmpFiles(self, name, dst, suh=None, tmpfiles=None, krb5_principal=None, replication=3):
        ...
        jobwaiter = HDFSJobWaiter("rename")
        for f in tmpfiles:
            src = hdfsPathJoin(base, f.path)
            path = src[:-len(suffix)]
            hj = self.createHadoopCliJob(src, krb5_principal)
            hj.mv(src, path)
            jobwaiter.addJob(src, hj)

        try:
            jobwaiter.wait(suh=suh)
        except Exception as e:
            logger.exception('Failed to rename temp files on HDFS')
    ...
    def removeFiles(self, dst, files, krb5_principal=None):
        base      = getBaseURI(dst)
        jobwaiter = HDFSJobWaiter("remove")
        for f in files:
            src = hdfsPathJoin(base, f.path)
            hj = self.createHadoopCliJob(src, krb5_principal)
            hj.rm(src)
    ...
    def cleanup(self, name, dst, sid, child_sid, throwMsg='', replication=3):
        rm_total_files = 0
        rm_total_time = 0
        fn_total_files = 0
        fn_total_time = 0
        ...
        if rm_total_files>0 or fn_total_files>0:       
            logger.info("Cleanup process was run: export_name=%s, sid=%s, child_sid=\"%s\", throwMsg=%s"  % (name, sid, ','.join(child_sid), throwMsg))
            logger_metrics.info("group=export, export_name=\"%s\", sid=%s, child_sid=\"%s\", cleanup_rm_files=%d, cleanup_rm_time=%.3f, cleanup_moved_files=%d, cleanup_move_time=%.3f" 
                                % (name, sid, ','.join(child_sid), rm_total_files, rm_total_time, fn_total_files, fn_total_time))
    
        if len(throwMsg) > 0:
           raise Exception(throwMsg)
    
        # list cursors again and ensure that no partial cursors are left behind
        cursors = self.getCursors(name, dst)
        for c in cursors:
            if c['state'] != None: 
               raise Exception('Found unexpected partial state cursor: ' + c['path'])
    ...
    def waitForSearches(self, jobs, info, sid, name, suh=None, period=1):
        ...
        while not canceled and len(waitJobs) > 0:
           ...
           if counter % 10 == 0 and suh!=None:
              ...
              if counter % 120 == 0:
                  logger.info("waiting for the count=%d search jobs to complete: export_name=%s, sid=%s, child_sid=\"%s\" " % ( len(waitJobs), name, sid, ','.join([sj.id for sj in waitJobs]) ) )
           ...
           for sj in waitJobs:
              ...
              try:
                  ...
                  msg.append('Error while waiting for export search sid=%s to complete. %s'  % (sj.id, str(e)))
                  canceled = True
                  try:
                     sj.cancel()
                  except:  pass
        ...
        if len(msg) > 0:
           for m in msg:
              info.addErrorMessage('Export search message: ' + str(m))
    ...
    def validateRunExport(self, namespace, owner, sessionKey, argvals, sid):
        ...
        if not 'name' in argvals:
           raise Exception('No export job specified, please specify one using name=<export-job-name>')
        ...
        if not sid.startswith('scheduler_') and not unquote(argvals.get('forcerun', '')) == '1':
           raise Exception('runexport is only meant to be ran by scheduled searches. Add forcerun=1 to the command to disable this check, but please make sure the search is not interfering with any scheduled searches')
    ...
    def removeWAL(self, name, dst, raiseOnError=True):
        ...
        hj.rm(wal_hdfs_path)
        if hj.wait() != 0:
            raise Exception('Failed to remove WAL file from HDFS, path=%s. Error: %s' % (wal_hdfs_path, hj.getStderr()))
    ...
    def writeWAL(self, name, dst, tmpfiles):
        ...
        if hj.wait() != 0:
            raise Exception('Failed to write WAL to HDFS, path=%s. Error: %s' % (wal_hdfs_path, hj.getStderr()))
    ...
    def readWAL(self, name, dst):
        ...
        if hj.wait() != 0:
            raise Exception('Failed to get WAL file from HDFS, path=%s. Error: %s' % (wal_hdfs_path, hj.getStderr()))
    ...
    def readExportFieldsFromHdfs(self, name, dst, format):
        ...
        if hj.wait() != 0:
            raise Exception('Failed to get export fields header file from HDFS, path=%s. Error: %s' % (header_hdfs_path, hj.getStderr()))
    ...
    def writeExportFieldsToHdfs(self, name, dst, fields, format):
        ...
        if hj.wait() != 0:
            raise Exception('Failed to write export fields header file to HDFS, path=%s. Error: %s' % (header_hdfs_path, hj.getStderr()))
    ...
    def verifyExportFields(self, name, dst, format, fields):
        ...
        if format == '':
            raise Exception('argument format cannot be empty')
        if fields == '':
            raise Exception('argument fields cannot be empty')
       
        if format not in supported_formats:
           raise Exception('format=%s is not supported, must be one of [%s]' % (format, ','.join(supported_formats)) ) 
        ...
        if format == 'csv' or format == 'tsv':
            ...
            for f in export_fields: 
                if f.find('*') >= 0:
                    raise Exception("Wildcarded fields are not allowed for format=%s, field=%s" % (format, f))
            ...
            if len(last_fields) == 0: # exporting for the first time
                ...
                for field in export_fields:
                    if field not in last_fields:
                        raise Exception("Field name=%s not present in fields exported last time list=%s" % (field, str(last_fields)))
    ...
    def main(self, results, settings, info, suh):
        ...
        peers     = entity.getEntities('search/distributed/peers', search="disabled=0", namespace=namespace, owner=owner, sessionKey=sessionKey) 
        peersDown = []
        peersAll  = ['local']    
        ...
        if len(peersDown) > 0:
            ...
            raise Exception('Found %d peer(s) enabled but not "Up", cannot run export job until all peers are "Up". Peers: %s' % (len(peersDown), names)) 
        ...
        if jc == 1 or len(peersAll) == 1:  
           searches.append( buildSearchString('', index_times, search, evalSearch, sid, basefilename, name, roll_size, dst, principal, format, export_fields, required_fields) )
           ...
           for pl in splitList(peersAll, jc):
              ...
              searches.append( buildSearchString('(' + splunk_servers + ')', index_times, search, evalSearch, sid, bfn, name, roll_size, dst, principal, format, export_fields, required_fields) ) 
              #log the export jobs parameters
              logger.info('export args: starttime=%d, endtime=%d, minspan=%d, maxspan=%d, roll_size=%d, format=%s, fields="%s", search="%s", %s' 
                          % (starttime, lt, minspan, maxspan, roll_size, format, export_fields, search, principal))
              i += 1
        ...
        if len(msg) > 0 :
           ...
           self.cleanup(name, dst, sid, child_sid, 'Canceled export search(es) due to %d error(s), child_sid=\"%s\"'% (len(msg), ','.join(child_sid) ))
        ...
        try:
            ...
            if(isinstance(e, RenameException)):
        ...
        logger_metrics.info("group=export, export_name=\"%s\", sid=%s, child_sid=\"%s\", search_time=%.3f, moved_files=%d, move_time=%.3f, total_size_mb=%d" 
...
if __name__ == '__main__':
    ...
    try:
        ...
        if isinstance(e, InfoSearchException):
            logger.info(str(e))
            info.addInfoMessage(str(e))
        else:
            logger.exception(str(e))
            info.addErrorMessage(str(e))
        info.finalizeSearch()
        if suh.content['status'] != 'done':
            suh.updateFailedStatus(str(e))
    finally:
        try:
            info.writeOut()
        except Exception, e:
