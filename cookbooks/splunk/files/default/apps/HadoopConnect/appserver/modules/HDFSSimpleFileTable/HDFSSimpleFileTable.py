...
class HDFSSimpleFileTable(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, count=1000,
        ...
        for i, result in enumerate(dataset):
            ...
            output.append('data-Location="%s">' % (result.get('Location')))
            ...
            for field in fieldNames:
                ...
                if fieldValues:
                    ...
                    # however for single values the extra div is unwanted.
                        ...
                        if field=="Type":
                            output.append("<img src=\"/static/app/HadoopConnect/images/%s_icon.png\"/>" % (fieldValues[0]))
                        else:
                    ...
                    output.append('></td>')
            indexHDFSManager = util.make_url(['manager',client_app,'data','inputs','hdfs','_new'], {'ns': client_app, 'action': 'edit', 'def.name': re.sub('^hdfs://', '', str(result.get('Location')))})
            previewURI = util.make_url(['app','search','flashtimeline'], {'q': '| hdfs read %s' % result.get('Location')})
            output.append("<td>")
            actions = []
