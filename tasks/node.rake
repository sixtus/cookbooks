...
namespace :node do
  ...
  task :bootstrap, :fqdn, :ipaddress, :password do |t, args|
    ...
    sh("knife bootstrap #{args.fqdn} --no-host-key-verify --no-node-verify-api-cert --node-ssl-verify-mode none --distro #{ENV['DISTRO']} -P #{args.password} -r 'role[#{ENV['ROLE']}]' -E #{ENV['ENVIRONMENT']} -i #{key}")
  end
